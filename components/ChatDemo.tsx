...
export default function ChatDemo() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-slate-950 text-white">
      <section className="relative mx-auto grid min-h-screen max-w-7xl items-center gap-10 px-5 py-8 lg:grid-cols-[0.92fr_1.08fr] lg:px-8">
        <div>
        ...
        <div className="rounded-[2rem] border border-white/10 bg-white/10 p-3 shadow-soft">
          <div className="grid gap-3 rounded-[1.5rem] bg-slate-100 p-3 text-slate-950 md:grid-cols-[1.1fr_0.8fr]">
            <section className="flex min-h-[620px] flex-col rounded-[1.25rem] bg-white shadow-sm">
                    ...
                    return (
                      <div key={message.id} className={`flex gap-2 chat-message ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                        {message.role === "assistant" && (
                          <div className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-slate-950 text-white mt-1">
                            <Bot size={15} />
                          </div>
                        )}
                        ...
                        <div className={`max-w-[82%] rounded-2xl px-4 py-3 text-sm leading-6 ${message.role === "user" ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-700"}`}>
                          ...
                          {showTimestamp && message.timestamp && (
                            <div className={`text-xs mt-1.5 ${message.role === "user" ? "text-blue-200" : "text-slate-500"}`}>
                        ...
                        {message.role === "user" && (
                          <div className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-blue-100 text-blue-700 mt-1">
                            <UserRound size={15} />
                          </div>
                        )}
                      </div>
              ...
              <div className="border-t border-slate-100 p-4">
                ...
                >
                  ...
                  />
                  <button className="grid h-12 w-12 place-items-center rounded-2xl bg-slate-950 text-white transition hover:bg-blue-700" aria-label="Send">
                    <Send size={18} />
                  </button>
                </form>
              </div>
            ...
            <aside className="rounded-[1.25rem] bg-slate-950 p-5 text-white">
